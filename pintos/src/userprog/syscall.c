...
static int sys_mmap(void* esp)
{
  int mapid,addr;
  if(read_arg((esp+sizeof(int)),&mapid)==-1||read_arg((esp+sizeof(int)*2),&addr)==-1)
    {
      thread_current()->exit_status=-1;
      thread_exit();
    }
  if (mapid<1)
    return -1;
  return mmap(mapid,(void*)addr);
}
static int sys_munmap(void* esp){
  return 0;
}
void
syscall_init (void) 
